...
class AnycubicMqtt:
    ...
    def publish(self, topic: str, payload: str) -> None:
        # paho reports a send into a dead socket in the RETURN VALUE, not by raising.
        # Discarding it is how a poll could keep "succeeding" against nothing.
        info = self._c.publish(topic, payload)
        if getattr(info, "rc", mqtt.MQTT_ERR_SUCCESS) == mqtt.MQTT_ERR_NO_CONN:
            self._connected = False
